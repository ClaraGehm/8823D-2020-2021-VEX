...
void drive_fwd_to_ball(double aproxEncoderTicks) //aproxEncoderTicks will serve as a way to shut off the code just in case something happens
{
  intake_open(0.3); 
  ...
  leftE.setPosition(0, degrees); //resets the value of the encoders
  rightE.setPosition(0, degrees); 

  while (locator.objectDistance(inches) > 5 || end == 1) //the number 5 represents how far away the ball needs to be in order for it to be in the system and the end variable serves as a failsafe to make sure that the robot moves on just in case it gets stuck or it is off
  {
    i++;
    ...
    if(leftE.position(degrees) > aproxEncoderTicks && -rightE.position(degrees) > aproxEncoderTicks)  //if the robot does not recognize the ball it will still stop at the apropriate place
    {
      end = 1;
    }
