...
double target;
double error, error2, errorDiffrence;
double prevError; 
int speed, speed2;
double currentDist, dist, currentDist2;
double totalDist;
double speedCap = 60;
...
void drive_fwd(int dist)
{
  ...
  while (Brain.Timer.value() < .3)
  {
    ...
    i++;
    currentDist = leftEncoder.value();         //determining how far the robot has traveled
    currentDist2 = rightEncoder.value();
    error = dist - currentDist;             //setting error to equal the distance remaining (negative number)
    error2 = dist - currentDist2;
