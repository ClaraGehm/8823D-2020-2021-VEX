...
///////////////////////////Proportional Turn variables//////////////////////////////////////////////////////////////////
  ...
  double turnkP = .63;     //tuning value for our turnError.
  double turnkD = .1;
  double turnSpeed = 0; // motor speed, starts at zero.
  double turnError = 0; // the difference between our turnTarget and our current position
...
{ 
  ...
  while(avgRotation < target -1) // loop runs until robot has almost reached target.
  { 
    ...
    turnSpeed = (turnError*turnkP) + (turnDerivative*turnkD); //tune error with kP and assign value to turnSpeed.
...
void drive_fwd(int dist)
{
  ...
  diffrencekP = .048;                    //.8 did not jerk
...
{
  Brain.Timer.reset();
    while(BallDetect.value() != 1 && Brain.Timer.value() < 3)
  {
    rightIntake.spin(forward, 100, percent);
    leftIntake.spin(forward, 100, percent);
    indexerMotor.spin(forward, 100, percent);
  }
  wait(200, msec);
  rightIntake.stop();  
  leftIntake.stop();
...
void autonomous(void) 
{
  ...
  drive_fwd(850); //value between intaking first ball and turning before goal
  drive_tl(-70);
  wait(.35, sec);
  leftIntake.stop();
  rightIntake.stop();
  ...
  rightIntake.stop();
  intake_open();
  drive_fwd(860);                               ///////////LAST VALUE ADJUSTED TESTTTTTTTTTTTTTTTTTTTTTTT
  flywheel.stop();
  ...
  intake_open();
  drive_fwd(500);
                                                                                    //after third goal back up and grab ball behind then when backing out 
                                                                                    //of fourth goal grab the one closest behind
  ...
  //            FOURTH GOAL            
  //////////////////////////////////////////////////////////////////////
  //back away from goal, full 360, open intakes
  drive_bwd(30, 300);
  drive_tr(180);
  intake_open();

  //move forward and pick up ball then open intakes
  drive_fwd(600);
  pickupBall();
  intake_open();

  //forward after ball pick up 
  drive_fwd(700);

  //intake bottom ball out of goal and shoot red ball
