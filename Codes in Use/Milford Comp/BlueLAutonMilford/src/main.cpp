...
double
    turnError; // the difference between our turnTarget and our current position
/*---------------------------------------------------------------------------*/
...

{
  ...
  while (InertialSensor.rotation(degrees) <
         target) // loop runs until robot has reached target.
  {
    ...
    turnSpeed = (turnError *
                 turnkP); // tune error with kP and assign value to turnSpeed.
  ...
  while (turnError < 0) { // if and when robot overshoots, run a smaller
...

{
  ...
  while (InertialSensor.rotation(degrees) >
         target) // loop runs until robot has reached target.
  {
    Brain.Screen.printAt(50, 50, "Inertial: %f",
...
void ballKickout() {
  ...
  if (DistanceSensor.objectDistance(inches) < 0.5) {
    if (OpticalSensor.hue() >= 225 && OpticalSensor.hue() <= 245) // look for blue
    {
...
void usercontrol(void) {
  ...
  while (1) { // This is for the driving motors
    ...
    // Drive Control
    // Control layout is R/C with dedicated joysticks for moving
    // forward/backward and rotating left/right The sum of the inputs is the
    // mapped to an array to control motor power
    ...
    if (Controller1.ButtonL2.pressing()) {
      ...
      leftIntake.setStopping(coast);
                 ...
                 .pressing()) //|| (Controller1.ButtonL1.pressing() && r == -1))
                              ////movs out 20 degrees
    {
