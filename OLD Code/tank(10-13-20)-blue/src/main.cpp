...
bool y4ball = eyes.objects[-100].centerY;
bool x4ball = eyes.objects[0].centerX;

void ballKickout()
{
  if(kill == 1)
  {
    if (eyes.takeSnapshot(eyes__BLUEB) && y4ball && x4ball)
    {
      flywheel.spin(forward);
    }
    else if (eyes.takeSnapshot(eyes__REDB) && y4ball && x4ball)
    {
